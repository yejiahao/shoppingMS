...
public class UserService extends HttpServlet {
    ...
    public void userLogin(HttpServletRequest request, HttpServletResponse response, Map<String, String> userMap) throws ServletException, IOException {
        ...
        handleCookies(request, response, userMap.get("username"), userMap.get("userpass"), userMap.get("isCookie"));// 处理cookies信息

        // 为了使用map，也是醉了   另外：map取值时对大小写敏感
        Map<String, Object> map = new Hashtable<>();
        Map<String, String> namePass = new Hashtable<>();
    ...
    public String isLogin(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 判断用户是否登陆
        String user = username.getUsername();// 登陆者的用户名
     ...
     * 登陆失败
     */
    public void fail(HttpServletResponse response, String backNews) {
        try {
            PrintWriter out = response.getWriter();
