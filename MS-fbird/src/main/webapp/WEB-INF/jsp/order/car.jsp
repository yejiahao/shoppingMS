            ...
            <table class="tab">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <tbody>
                        <tr>
                            <th><a class="car-batch" href=""><i class="fa fa-cart-plus fa-lg fa-red"> 结算</i></a></a>
                            </th>
                            <th>序号</th>
                            <th>商品名称</th>
                            <th>商品价格</th>
                        ...
                        <c:forEach var="item" items="${carList}" varStatus="idx" begin="0" end="${fn:length(carList)}">
                            <tr>
                                <td><input type="checkbox" name="carBuyId" checked="checked" value="${item.gCarGId}">
                                </td>
                                <td>${idx.index + 1}</td>
                                <td>${item.gCarGName}</td>
                                <td><i class="fa fa-jpy"> ${item.gCarPrice}</i></td>
                                <td><input type="number" id="carNumber_${idx.index}" value="${item.gCarNumber}" min="1"
                                           max="10"></td>
